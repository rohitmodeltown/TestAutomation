...
public class Auth extends Helper {
	public String vpa;
	...
	public String PK_Auth;
	public String beneAcctNo;
	public String merchantCode = "MER0000000000005";
	public String callBackUrl = "dynamic.upi.qr.callback.url";
	public String airtelIfscNo = "AIRP0000001";
	...
	public void initializeValues() {
		...
		partnerName = "AIRTELPOSTPAID";
		PK_Auth = partnerName + "_"+circleCode;
		...
		bin6 = new Bin("beneAccNo", beneAcctNo);
		bin7 = new Bin("partnerName", partnerName);
		(new Aspike(nameSpace, setName2, PK_Auth)).insertRecord(bin4, bin6, bin7, bin5);
	}
	...
	@Test
	public void firstTimeCustomerSuccessTest() {
		...
		assertEquals(response.path("data.bankDetails"), beneAcctNo+" | "+ airtelIfscNo);
		assertEquals(response.path("data.status"), null);
		assertEquals(response.path("data.statusCode"), null);
		assertEquals(response.path("data.mccCode"), mccCode);
		assertEquals(response.path("data.merchantCode"), merchantCode);
		assertEquals(response.path("data.callBackUrl"), callBackUrl);
		...
		assertEquals(rec.bins.get("beneAccNo"), beneAcctNo);
		assertEquals(rec.bins.get("mccCode"), mccCode);
	}
	
	@Test
	public void repeatCustomerFullEntrySuccessTest() {
		(new Aspike(nameSpace, setName1, vpa)).insertRecord(bin1, bin2, bin3, bin4, bin5, bin6);
	...
	@Test
	public void repeatCustomerPartialEntrySuccessTest() {
		// delete record
		(new Aspike(nameSpace, setName1, vpa)).deleteRecord();
	...
	public void invalidFeSessionIdTest() throws InterruptedException {
		// abbreviation other than pp or fl is invalid
		feSessionId = "222222222255555555559";
		Response response = given()
				.header("reqType", reqType).header("feSessionid", feSessionId).header("password", getPassword(feSessionId, vpa, salt)).header("channel", channel)
				.header("username", userName).contentType(contentType).log().all().when().get(getUrl(vpa)).then().log()
				.all().extract().response();
